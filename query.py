...
class query:
    ...
    def query(self,url,api_key=None,search_dat = None,search_settings = None):
        if search_dat is not None:
            url = 'https://wallhaven.cc/api/v1/search?'
            url = self.url_tag_parser(url,search_dat)

        if search_settings is not None:
            url = self.add_search_settings(url,search_settings)
    ...
    def add_search_settings(self,url,search_settings):
        for elm in search_settings:
            url = f"{url}&{elm}"
